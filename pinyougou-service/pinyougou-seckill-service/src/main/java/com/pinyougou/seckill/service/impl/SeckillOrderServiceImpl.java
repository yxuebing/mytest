...
public class SeckillOrderServiceImpl implements SeckillOrderService {
    ...
    @Override
    public List<SeckillOrder> findOrederTimeOut() {
        // 定义集合存储超时的未支付订单
        List<SeckillOrder> timoOutOrder = new ArrayList<>();
        //  查询redis中的未支付的订单
        List<SeckillOrder> seckillOrders = redisTemplate.boundHashOps("seckillOrder").values();
        if (seckillOrders != null && seckillOrders.size() > 0) {
            for (SeckillOrder seckillOrder : seckillOrders) {
                //   将当前时间减去五分钟得到的时间与 未支付订单的时间做对比,大了就是超时了
                Long time = new Date().getTime() - (5*60 * 1000);
                if (time > seckillOrder.getCreateTime().getTime()) {
                    timoOutOrder.add(seckillOrder);
                }
            }
     ...
     */
    @Override
    public void deleteOrderForRedis(SeckillOrder seckillOrder) {
            ...
            // 从redis中查询秒杀商品
            SeckillGoods seckillGood = (SeckillGoods) redisTemplate.boundHashOps("seckillGoodsList").get(seckillOrder.getSeckillId());
                        // 代表商品秒光
                 if(seckillGood!=null ){
                     // 修改缓存中秒杀商品的库存
                     ...
                     seckillGood = seckillGoodsMapper.selectByPrimaryKey(seckillOrder.getSeckillId());
                     seckillGood.setStockCount(1);

            }

                redisTemplate.boundHashOps("seckillGoodsList")
                        .put(seckillOrder.getSeckillId(),seckillGood);
